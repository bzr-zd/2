...
var rule = {
    ...
    class_name: '臻彩4K🌈电影&臻彩4K🌈电视剧&臻彩4K🌈综艺&臻彩4K🌈动漫&臻彩4K🌈少儿&臻彩4K🌈纪录片',
    ...
    二级: $js.toString(() => {
        ...
        if (/get_playsource/.test(input)) {
            ...
            indexList.forEach(function(it) {
                ...
                vdata.forEach(function(item) {
                    if (!item.title.includes("预告") && !item.title.includes("花絮")) {
                        d.push({
                            title: item.title,
                            pic_url: item.pic,
            ...
            if (video_lists.length === 1) {
                ...
                video_list.forEach(function(it, idex) {
                    ...
                    QZOutputJson.results.forEach(function(it1) {
                        it1 = it1.fields;
                        // 这里也加上预告和花絮的过滤
                        if (!it1.title.includes("预告") && !it1.title.includes("花絮")) {
        ...
        let yg = []; // 强制设置为空，不再收集预告
        let zp = d.filter(function(it) {
            // 保留所有非空链接作为正片（或者根据需要进一步过滤）
            return it.type !== "预告" && it.type !== "花絮" 
        });
        
        // 既然不想要预告分组，那么逻辑简化：
        // 无论如何只显示 "开心果影视4K"
        VOD.vod_play_from = "开心果影视4K";
